...
function clickAnim(a,b){null==b&&(b=!0);for(key in a)a[key]=assets.bgimages[a[key]];engine.ports.cycledClickAnim.send([a,b]);engine.ports.proceedRequest.send(!0)}function clickAnimOnly(a,b){null==b&&(b=!0);hideScripts();for(key in a)a[key]=assets.bgimages[a[key]];engine.ports.cycledClickAnim.send([a,b]);engine.ports.isModal.send(!0)}function show(a){engine.ports.cycledClickAnim.send([[assets.bgimages[a]],!0]);engine.ports.proceedRequest.send(!0)}
...
function playStop(a){for(key in a)soundManager.stop(a[key]);engine.ports.proceedRequest.send(!0)}function showStandPicts(a){var b=[];for(key in a)b.push({name:key,imageInfo:assets.fgimages[a[key]]});engine.ports.showPicts.send(b);engine.ports.proceedRequest.send(!0)}function changeStandPict(a,b){engine.ports.changePict.send({name:a,imageInfo:assets.fgimages[b]});engine.ports.proceedRequest.send(!0)}function restart(){location.reload()}
