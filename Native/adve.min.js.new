...
function clickAnim(a){for(key in a)a[key]=assets.images[a[key]];engine.ports.clickAnim.send(a);engine.ports.proceedRequest.send(!0)}function clickAnimOnly(a){hideScripts();for(key in a)a[key]=assets.images[a[key]];engine.ports.clickAnim.send(a);engine.ports.isModal.send(!0)}function show(a){engine.ports.clickAnim.send([assets.images[a]]);engine.ports.proceedRequest.send(!0)}
...
function gotoScript(a,b){thisScript=a;thisSection=b;"undefined"==typeof b&&(b="#begin");$("#hiddenplace").load(a,null,function(){var a=$(b),c=expandMacro(a.text()).replace(/\uff5c([^\u300a]+)\u300a([^\u300b]+)\u300b/g,"<ruby>$1<rp>(</rp><rt>$2</rt><rp>)</rp></ruby>").replace(/\uff56/g,"<span style='color:red'>&hearts;</span>").split("\n");a.empty();engine.ports.newScript.send(c)})}function gotoSection(a){gotoScript(thisScript,a)}function repeatSection(){gotoScript(thisScript,thisSection)}
